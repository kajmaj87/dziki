...
for i, dx in track(enumerate(docs), "Processing", len(docs)):
    ...
    for j, dy in enumerate(docs):
        if j>i:
            simt = dx['nlpt'].similarity(dy['nlpt']) if len(dx['text'])>TEXT_MIN_LENGHT and len(dy['text'])>TEXT_MIN_LENGHT else 0
            if simt>0.7 and (dx['verdict'] == 'false' or dx['verdict'] == 'true'):
                simc = dx['nlpc'].similarity(dy['nlpc']) if len(dx['comment'])>TEXT_MIN_LENGHT and len(dy['comment'])>TEXT_MIN_LENGHT else 0
                simt = simt if simt < 1 else 2
                similarities.append((simt,simc,simt**2+simc**2,dx['id'],"Dup:"+str(dx['dup'])+"_v:"+dx['verdict'],dy['id'],"Dup:"+str(dy['dup'])+"_v:"+dy['verdict'],dx['text'],dy['text'],dx['comment'],dy['comment']))
