...
def test_process_documents_should_return_single_element_array_when_passed_two_documents():
    ...
    assert processDocuments(DummyNlp(), [doc1, doc2], fields=[{"name": "text"}]) == {
        "similarities": [{"similarities": {"text": DEFAULT_SIMILARITY}, "docs": [doc1, doc2]}]
    }
...
def test_process_documents_should_return_single_element_array_when_passed_two_documents_with_two_fields():
    ...
    assert processDocuments(DummyNlp(), [doc1, doc2], fields=[{"name": "text"}, {"name": "comment"}]) == {
        "similarities": [
            {"similarities": {"text": DEFAULT_SIMILARITY, "comment": DEFAULT_SIMILARITY}, "docs": [doc1, doc2]}
        ]
    }
